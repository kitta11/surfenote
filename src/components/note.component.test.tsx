...
function moveCursor(element: Node, position: number) {
  ...
  range.setStart(
    targetNode,
    Math.min(position, targetNode.textContent?.length ?? 0),
  )
  range.collapse(true)
...
describe('Note', () => {
  ...
  it('should be rendered with content', () => {
    ...
    expect(
      screen.getByTestId(`note-${noteMockData2.id}-body`),
    ).toHaveTextContent(noteMockData2.body)
  })
  ...
  it('should show an overlay when user hits @ character', async () => {
    ...
    expect(
      screen.getByTestId(`note-${noteMockData1.id}-mention-overlay`),
    ).toBeInTheDocument()
  })
})
